...
class ViT_AdptFormer_Model(nn.Module):
    def __init__(self, input_size, num_classes=5):
        ...
        self.fc_transform2 = nn.Linear(embed, 128)
        self.our_mlp_proj = nn.Sequential(*[nn.Linear(128+num_classes, self.dimension),nn.ReLU(inplace=False),nn.Linear(self.dimension, self.dimension)])
        self.mb_classifiers = nn.Linear(self.dimension, num_classes * 2, bias=True)
    ...
    def forward(self, x,labels=None, update=None):
        ...
        prompt_out = (features @ self.prompt.transpose(-2, -1)* self.scale)
        
        out = self.fc_head(image_features)
        
        
        mb_features = self.fc_transform2(image_features)
        feat_proj = self.our_mlp_proj(torch.cat((mb_features, out), dim=1))
        logits_mb = self.mb_classifiers(feat_proj) #[bsz, 2K]
        
        return out,prompt_out,features,self.prompt,logits_mb
