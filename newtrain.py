...
def train():
    ...
    num_classes = 4
    batch_size = 46
    model_name = 'Vit_openOODbin_skin'
    save_path = './new/'  
    image_size = (320,320)
    consistency = 1.0
    max_epoch = 50
    ...
    for name,p in model.named_parameters():
        if 'fc_head'  in name or 'our_mlp_proj'  in name or 'mb_classifiers'  in name:   # adaptmlp fc_transform2 positional_embedding
    ...
    batch_sampler,train_dataset,ID_test_dataset,OOD_test_dataset=get_dataset_splits(image_size,batch_size,'skin',[0,1,2,3,4],0.3, True)
    ...
    for epoch_num in range(max_epoch):
        ...
        for i_batch, (X,Y) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(): # Student Model
                ...
                entropy = -torch.mean(torch.sum(probs * torch.log2(probs), dim=1))#torch.mean(torch.sum(y*torch.log(y/x),dim=-1))    
                ...
                unlabel_consist_loss = F.mse_loss(unlabel_out_soft,ema_unlabel_out_soft)+F.mse_loss(label_out_soft,ema_label_out_soft)+\
                                       ...
                                       F.mse_loss(logits_mb_label,ema_logits_mb_label)+F.mse_loss(logits_mb_unlabel,ema_logits_mb_unlabel)+entropy
